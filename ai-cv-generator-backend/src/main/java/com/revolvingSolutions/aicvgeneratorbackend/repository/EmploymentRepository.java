...
public interface EmploymentRepository extends JpaRepository<EmploymentEntity,Integer> {
    @Query("SELECT new com.revolvingSolutions.aicvgeneratorbackend.model.Employment(f.company,f.title,f.startdate,f.enddate) FROM EmploymentEntity f WHERE f.user.username = ?1")
    public List<FileModel> getEmploymentHistoryFromUser(String username);
}
